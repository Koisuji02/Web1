...
export const getUser = (email, password) => {
  return new Promise((resolve, reject) => {
    ...
    db.get(sql, [email], (err, row) => {
      ...
      // se non trovato (non esiste l'utente)
      else if (row === undefined) { 
        resolve(false); 
      }
      // se trovato
        ...
        // verifica password facendo uso di scrypt e timingSafeEqual (come slides):
        // - scrypt -> genera un hash della password con il salt dell'utente
        // - timingSafeEqual -> confronta l'hash della password inserita con quello salvato nel db
