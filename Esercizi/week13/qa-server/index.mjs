...
import {listQuestions, getQuestion, listAnswersOf, addAnswer, updateAnswer, voteAnswer,
  //! aggiunto per auth
  getUser} from './dao.mjs';
...
//! aggiunto per auth (serializzazione per session)
passport.serializeUser(function (user, cb) {
  cb(null, user);
});

//! aggiunto per auth (deserializzazione per session)
...
//! aggiunto per auth (impostazione della sessione)
app.use(session({
...
app.post('/api/questions/:id/answers', isLoggedIn, [
  ...
  // controllo che l'email della answer creata sia quella dell'utente loggato
