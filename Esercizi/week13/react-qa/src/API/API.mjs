...
const voteUp = async (answerId) => {
  const response = await fetch(`${SERVER_URL}/api/answers/${answerId}/vote`, {
    ...
    //! si mette credentials: 'include' per fare in modo che si verifichi se l'utente è loggato anche lato client (importante farlo!!!)
    credentials: 'include'
  });
...
//! aggiunto per auth
// fa una richiesta al server per ottenere le informazioni dell'utente loggato (specchiata con la get current in index.mjs)
